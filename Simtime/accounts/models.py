...
def user_path(instance, filename):  # instance는 Photo 클래스의 객체, filename은 업로드할 파일의 파일이름
    ...
    # user create시 instance.host는 없다. 그냥 instance로 하면
...
def user_profile_path(instance, filename):
    ...
    # ex) honux/asfqqwer.png
    # user create시 instance.host는 없다. 그냥 instance로 하면
    return '%s/profile.%s' % (instance, extension)


# instance는 Photo 클래스의 객체, filename은 업로드할 파일의 파일이름
...
class Account(AbstractUser):
    ...
    profile_image = models.ImageField(
        upload_to=user_profile_path, default='user-basic.png', blank=True, null=True)
